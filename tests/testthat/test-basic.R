...
sum_vars <- c("num_S","num_I","num_I_strains","mean_alpha","sd_alpha",
              "mean_beta","sd_beta","shannon")
m <- colMeans(tail(res[sum_vars],10))
expect_equal(m,
             c(num_S = 89.8, num_I = 310.2, num_I_strains = 15.9,
               mean_alpha = 0.121693099287746, sd_alpha = 0.0183743633406564,
               mean_beta = 0.0493277969025787, sd_beta = 0.00251319702486456,
               shannon = 1.79367083406878))

## plot (not part of a real pipeline)
alpha_vars <- res[,c("mean_alpha","median_alpha","lower_alpha","upper_alpha")]
alpha_vars$mean_alpha <- log(alpha_vars$mean_alpha)
matplot(res$time,
       alpha_vars,
       type="l",
       xlab="time",
